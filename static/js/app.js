...
app.controller('translatrController', function ($scope, $http, $timeout) {
	...
	$scope.locales = {
		...
		$scope.validateAndTranslate = function () {
			...
			if (!$scope.userText) {
				$scope.errorText = "Merci d'entrer un text valide";
				return;
			}

			if (!l.length) {
				$scope.errorText = 'Merci de selectionner au moins une langue.';
				return;
			}
			...
			$scope.errorText = 'Je Crunch la donnée.';
			$http.post('', params).then(function (config) {
				...
				function () {
					$scope.isFetchingData = false;
					$scope.isPositive = false;
					$scope.errorText = 'Merci de recharger, une erreur a ete detectee.';
				})
			;
		};
